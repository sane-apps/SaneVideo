        ...
        autoreleasepool {
            ...
            if timeCoordinator.startTimeNeedsRecalibration, source == currentSource {
                ...
                if let w = videoWriter, !w.isWriting {
                    AppLogger.recording.error("Drop: videoWriter is NOT writing. Error: \(String(describing: w.error))")
                }
            }
            ...
            guard !isPaused, isRecording, currentSource == source, let writer = videoWriter, writer.isWriting else { return }
    ...
    @RecordingActor func processAudioSample(_ sampleBuffer: CMSampleBuffer) {
        autoreleasepool {
            ...
            if timeCoordinator.startTime == .zero {
                ...
                AppLogger.recording.info("Recording started (Mic Audio first). First sample time: \(presentationTime.seconds)")
            }

            if timeCoordinator.startTime != .zero, bufferToWrite.presentationTimeStamp >= timeCoordinator.startTime {
    ...
    @RecordingActor func processSystemAudioSample(_ sampleBuffer: CMSampleBuffer) {
        autoreleasepool {
            ...
            if timeCoordinator.startTime == .zero {
                ...
                AppLogger.recording.info("Recording started (System Audio first). First sample time: \(presentationTime.seconds)")
            }
