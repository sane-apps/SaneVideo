...
class AppState {
  ...
  func stopRecording() {
    ...
    if args.contains("-ui_testing") && args.contains("-open_editor") {
      ...
      fatalError(
        "TEST FAILURE: Unexpected stopRecording call during Export Workflow! Check console/crash logs."
      )
    }
    ...
    recordingState.stopRecording { [weak self] url in
      ...
      if let url = url {
        ...
        let isTesting =
          TestEnvironment.isUITesting || UserDefaults.standard.bool(forKey: "ui_testing")
  ...
  func toggleScreenShare() {
    ...
    if wasScreenSharing {
      ...
      windowManager.updatePiPState(
        isCameraActive: cameraState.isActive, isRecording: recordingState.isRecording)

      // FIX: Removed implicit stopRecording() call.
      // User expects to switch back to camera, not stop.
      ...
      if !cameraState.isActive {
        ...
        cameraEnabled = true
        cameraState.startCamera()
        effectiveCameraActive = true  // Treat as active for immediate UI update
      }

      // FIX: Pass effectiveCameraActive so PiP window shows immediately
      windowManager.updatePiPState(
        isCameraActive: effectiveCameraActive, isRecording: recordingState.isRecording)
    ...
    if recordingState.isRecording {
      ...
      // FEEDBACK: Show toast so user knows mode changed (since we removed the confusing 'Stop')
  ...
  func importVideo() {
    ServiceContainer.shared.logManager.logUserAction(
      "Action Executed", details: "Show Import Picker")
    projectState.showImportPicker()
  }

  func startNewRecording() {
    ServiceContainer.shared.logManager.logUserAction(
      "Action Executed", details: "Start New Project")
    projectState.startNewProject()
  }
  ...
  func addVideoToTimeline(url: URL) {
    ServiceContainer.shared.logManager.logUserAction(
      "Action Executed", details: "Add Video: \(url.lastPathComponent)")
    Task { @MainActor in
  ...
  func switchToRecording() {
    ...
    if ProcessInfo.processInfo.arguments.contains("-ui_testing") {
      ...
      fatalError(
        "TEST FAILURE: Unexpected switch to recording mode! Check console logs for stack trace.")
    }
  ...
  func saveCurrentState() {
    ...
    // Note: Scroll/Zoom would need to be passed in from View binding or TimelineState if available.
    // For now, passing 0/1.0 if not available, OR we can extend TimelineState to track this.
    // Since I haven't implemented TimelineState scroll tracking yet, I'll pass 0.0 for now,
    // but `currentTime` is the critical part.
    // Actually, if I pass 0.0, I reset scroll every time. That's annoying.
    // I should read from `projectState.currentProject.scrollOffset` as fallback if I don't have new value.
