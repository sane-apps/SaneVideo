...
extension RecordingEngine {
    ...
    func switchSource(source: RecordingSource) {
        Task { @RecordingActor in
            ...
            guard !isSwitching else {
                AppLogger.recording.warning("Source switch already in progress, ignoring request to switch to \(source)")
                return
            }
            ...
            AppLogger.recording.info("Source switch complete. Active: \(source)")
        }
    }
    ...
    private func performSourceSwitch(from _: RecordingSource, to newSource: RecordingSource) async {
        if newSource == .camera {
            ...
            AppLogger.recording.info("Switching to screen recording, keeping camera session active for PiP preview")
