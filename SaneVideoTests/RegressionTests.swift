...
final class RegressionTests: XCTestCase {
  ...
  // MARK: - Inspector Component Regression Tests

  /// Regression: Inspector should validate clip file existence before operations
  func testInspectorMissingFileValidation() {
    ...
    XCTAssertTrue(clip.isMissing, "Clip should be marked as missing when file doesn't exist")

    // In actual UI, operations would be disabled when clip.isMissing is true
  }

  /// Regression: Inspector error messages should be actionable
  func testInspectorActionableErrorMessages() {
    let errorMessage = "Cannot apply Magic Fix: Clip file is missing. Use 'Locate File' in Clip Info to relink the file."

    XCTAssertTrue(
      errorMessage.contains("Use 'Locate File'"),
      "Error message should include actionable guidance")
    XCTAssertTrue(
      errorMessage.contains("Clip Info"),
      "Error message should reference where to find the solution")
  }

  /// Regression: Inspector should handle clip deletion gracefully
  func testInspectorHandlesClipDeletion() {
    let videoURL = URL(fileURLWithPath: "/tmp/test.mp4")
    let clip = VideoClip(
      url: videoURL,
      duration: CMTime(seconds: 10, preferredTimescale: 600),
    ...
    var timeline = Timeline(tracks: [
      Track(name: "Test Track", type: .video, clips: [clip], zIndex: 0)
    ])

    // Delete clip
    timeline.tracks[0].clips.removeAll { $0.id == clip.id }

    // Verify clip no longer exists
    let clipExists = timeline.tracks[0].clips.contains { $0.id == clip.id }
    XCTAssertFalse(clipExists, "Clip should be removed from timeline")

    // In actual UI, validatedClip would return nil, causing auto-deselect
  }

  /// Regression: Inspector should sync state when clip properties change externally
  func testInspectorStateSync() {
    let videoURL = URL(fileURLWithPath: "/tmp/test.mp4")
    var clip = VideoClip(
      url: videoURL,
      duration: CMTime(seconds: 10, preferredTimescale: 600),
    ...
    XCTAssertEqual(clip.volume, 0.5, "Clip volume should be updated")

    // In actual UI, onChange(of: clip.volume) would sync the state
  }

  /// Regression: Inspector operations should be debounced to prevent excessive saves
  func testInspectorSliderDebouncing() async {
    ...
    // Verify only last value is set (in actual implementation, pending tasks are cancelled)
