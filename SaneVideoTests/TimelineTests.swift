...
final class TimelineTests: XCTestCase {
  ...
  func testMoveClip() {
    let clip1 = VideoClip(
      url: URL(fileURLWithPath: "/tmp/1.mp4"), duration: CMTime(seconds: 5, preferredTimescale: 600)
    )
    let clip2 = VideoClip(
      url: URL(fileURLWithPath: "/tmp/2.mp4"), duration: CMTime(seconds: 5, preferredTimescale: 600)
    )
    let clip3 = VideoClip(
      url: URL(fileURLWithPath: "/tmp/3.mp4"), duration: CMTime(seconds: 5, preferredTimescale: 600)
    )
  ...
  func testSplitClipLogic() {
    ...
    // Fix: Manually sequence the clips for the test context
    secondPart.startTime = firstPart.effectiveDuration
