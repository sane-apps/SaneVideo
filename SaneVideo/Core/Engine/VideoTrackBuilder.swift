    ...
    static func build(
        ...
        for track in visualTimelineTracks {
            ...
            guard let trackA = composition.addMutableTrack(withMediaType: .video, preferredTrackID: kCMPersistentTrackID_Invalid),
                  let trackB = composition.addMutableTrack(withMediaType: .video, preferredTrackID: kCMPersistentTrackID_Invalid) else { continue }
        ...
        var trackCursorData: [CMPersistentTrackID: URL] = [:]
        ...
        for (index, timelineTrack) in visualTimelineTracks.enumerated() {
            ...
            for (clipIndex, clip) in sortedClips.enumerated() {
                ...
                guard let assetTrack = try? await asset.load(.tracks).first(where: { $0.mediaType == .video }) else { continue }
                ...
                for (segIndex, segment) in validSegments.enumerated() {
                    ...
                    if clip.useSmoothCutForRemovals && segIndex > 0 {
                        ...
                        activeTransitions.append(TransitionMetadata(
                ...
                // A. Transform
                var layerConfig = AVVideoCompositionLayerInstruction.Configuration(trackID: currentCompTrack.trackID)
                ...
                if !clip.privacyRegions.isEmpty {
                    ...
                    regions.append((CMTimeRange(start: insertStart, duration: playDuration), clip.privacyRegions))
                    trackPrivacyRegions[currentCompTrack.trackID] = regions
                }

                // F. Cursor Data
                if clip.showCursorHighlight, let cursorURL = clip.cursorDataURL {
                    trackCursorData[currentCompTrack.trackID] = cursorURL
                }
