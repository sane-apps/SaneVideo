...
XCODEPROJ = File.join(PROJECT_ROOT, 'SaneVideo.xcodeproj')
SCHEME = 'SaneVideo'
...
def test_mode
  build
  app = Dir.glob(File.join(PROJECT_ROOT, 'build/**/SaneVideo.app')).first
  app ||= Dir.glob(File.expand_path('~/Library/Developer/Xcode/DerivedData/**/Build/Products/Debug/SaneVideo.app')).first
  abort 'Could not find built SaneVideo.app' unless app
  system('killall SaneVideo 2>/dev/null')
  sleep 0.5
  system("open '#{app}'")
  warn "Launched #{app}"
end
