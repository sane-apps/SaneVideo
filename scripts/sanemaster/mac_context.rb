...
module SaneMasterModules
  # Mac Developer Context Injection
  # Front-loads Claude with Mac-specific knowledge to prevent common mistakes
  module MacContext
    CONTEXT_FILE = '.claude/mac_context.md'

    # Generate and inject Mac development context at session start
    def inject_mac_context(_args = [])
      puts '🍎 --- [ MAC DEVELOPER CONTEXT ] ---'
      puts ''

      context = generate_mac_context
      context_path = File.join(Dir.pwd, CONTEXT_FILE)

      FileUtils.mkdir_p(File.dirname(context_path))
      File.write(context_path, context)

      puts "✅ Injected Mac context to #{CONTEXT_FILE}"
      puts "   #{context.lines.count} lines of Mac-specific knowledge"
      puts ''
      puts '💡 Claude will load this automatically via CLAUDE.md reference'
    end

    def show_mac_context(_args = [])
      puts generate_mac_context
    end
    ...
    def generate_mac_context
      <<~CONTEXT
        ...
        *Generated by SaneMaster mac_context module (v2.0 - WWDC 2025)*
      CONTEXT
    end
  end
end
