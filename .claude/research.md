...
**Updated:** 2026-05-17 | **Status:** verified | **TTL:** 14d
**Source:** Mac Mini/Finder runtime, full-screen screenshots in `~/Desktop/Screenshots/SaneVideo/`, App Store Connect API/Safari, `SaneMaster.rb verify`, `SaneMaster.rb release_preflight`, `SaneMaster.rb appstore_preflight`, `appstore_submit.rb`, `ffprobe`, local crash reports
...
- Sparkle sandbox release fix: the direct Info.plist must include `SUEnableInstallerLauncherService` for sandboxed Sparkle direct builds, while `Info-AppStore.plist` must stay free of all Sparkle keys. `ReleaseReadinessRegressionTests.testAppStoreLaneIsSeparateFromDirectSparkleLane` now guards both lanes; `./scripts/SaneMaster.rb verify` passed after the fix with `1198 tests` in `287s`.
