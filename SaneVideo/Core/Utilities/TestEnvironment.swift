...
/// Left-rail editor states an automation run can activate deterministically.
/// Raw values are the accepted `AUTOMATION_RAIL_STATE` / `-automation_rail_state` values.
enum AutomationRailState: String, CaseIterable {
    case transcript
    case thumbnail
    case voiceover
    case shorts

    /// The in-app notification that activates this rail state.
    /// Must match the names observed by SidebarView / MainContentView / GlobalSheetModifier.
    /// Note: `ShowSidebarTranscript` is automation-only — no UI control posts it (the
    /// Transcript rail item selects the tab directly); the others mirror notifications
    /// the matching Create rail buttons post when clicked.
    var notificationName: Notification.Name {
        switch self {
        case .transcript:
            Notification.Name("ShowSidebarTranscript")
        case .thumbnail:
            Notification.Name("GenerateThumbnail")
        case .voiceover:
            Notification.Name("GenerateVoiceover")
        case .shorts:
            Notification.Name("ShowRepurposingSheet")
        }
    }
}
...
enum TestEnvironment {
    ...
    static func shouldOpenEditor(
        ...
        arguments.contains("-open_editor")
            ...
            || automationRailState(arguments: arguments, environment: environment) != nil
    }
    ...
    /// Left-rail state automation should activate after the editor bootstrap settles.
    static var automationRailState: AutomationRailState? {
        automationRailState(
            arguments: ProcessInfo.processInfo.arguments,
            environment: env
        )
    ...
    static func automationRailState(in environment: [String: String]) -> AutomationRailState? {
        automationRailState(arguments: [], environment: environment)
    }

    static func automationRailState(
        arguments: [String],
        environment: [String: String]
    ) -> AutomationRailState? {
        // SANEVIDEO_AUTOMATION_RAIL_STATE exists because the shared launch tooling
        // forwards SANEVIDEO_-prefixed variables through LaunchServices.
        guard let rawValue = argumentValue(
            for: ["-automation_rail_state", "--automation-rail-state"],
            in: arguments
        ) ?? environment["AUTOMATION_RAIL_STATE"]
            ?? environment["SANEVIDEO_AUTOMATION_RAIL_STATE"]
        else {
            return nil
        }

        return AutomationRailState(
    ...
    static func logState() {
        ...
        if let railState = automationRailState {
            NSLog("🧪 [TestEnvironment] Automation rail state: \(railState.rawValue)")
        }
    }
}
