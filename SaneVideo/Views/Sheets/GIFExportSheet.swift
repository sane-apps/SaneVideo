...
struct GIFExportSheet: View {
    ...
    private var header: some View {
        HStack {
            VStack(alignment: .leading, spacing: 4) {
                Text(String(localized: "gif.header.title", defaultValue: "Export as GIF"))
                    .font(.headline)
                Text(String(localized: "gif.header.subtitle", defaultValue: "Create animated GIF from video"))
                    .font(.caption)
            ...
            .buttonStyle(.plain)
            .accessibilityIdentifier("gif.sheet.close")
        }
        .padding(16)
    }
    ...
    private var presetsSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            Label(String(localized: "gif.presets.header", defaultValue: "Quality Presets"), systemImage: "slider.horizontal.below.square.filled.and.square")
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 8) {
                ForEach(fpsOptions, id: \.self) { option in
                    ...
                    .tint(fps == option ? .blue : .secondary)
                    .accessibilityIdentifier("gif.fps.\(option)")
                }
            }
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 8) {
                ForEach(widthOptions, id: \.self) { option in
                    ...
                    .buttonStyle(.bordered)
                    .tint(width == option ? .blue : .secondary)
                    .accessibilityIdentifier("gif.width.\(option)")
                }
            }
        }
    }
    ...
    private var sizeEstimateSection: some View {
        HStack {
            Label(String(localized: "gif.size.estimate", defaultValue: "Estimated Size"), systemImage: "doc.fill")
                .font(.caption)
            Spacer()
            Text(estimatedSize)
    ...
    private var footer: some View {
        HStack {
            Button(String(localized: "gif.action.cancel", defaultValue: "Cancel")) {
                dismiss()
            }
            .keyboardShortcut(.cancelAction)
            .accessibilityIdentifier("gif.action.cancel")
            ...
            Button {
                ...
                if isExporting {
                    HStack {
                        ...
                        Text(String(localized: "gif.action.exporting", defaultValue: "Exporting..."))
                    }
                } else {
                    Label(String(localized: "gif.action.export", defaultValue: "Export GIF"), systemImage: "square.and.arrow.up")
                }
            }
            .buttonStyle(.borderedProminent)
            .disabled(isExporting)
            .keyboardShortcut(.defaultAction)
            .accessibilityIdentifier("gif.action.export")
        }
        .padding(16)
    }
    ...
    private func updateSizeEstimate() {
        // Rough estimation: GIF bytes ≈ width * height * fps * duration * 0.1 (compression factor)
        let duration = useFullVideo ? clipDuration : max(0.1, endTime - startTime)
        let height = Double(width) * 9.0 / 16.0 // Assume 16:9
        ...
        Task {
            do {
                ...
                guard await ffmpeg.isAvailable else {
                    await MainActor.run {
                        errorMessage = String(localized: "gif.error.ffmpeg_missing", defaultValue: "FFmpeg not installed. Run: brew install ffmpeg")
                        isExporting = false
                    }
                    return
                }
                ...
                await MainActor.run {
                    isExporting = false
                    ServiceContainer.shared.toastManager.show(String(localized: "gif.toast.exported", defaultValue: "✅ GIF exported!"))
