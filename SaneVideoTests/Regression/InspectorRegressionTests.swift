...
final class InspectorRegressionTests: XCTestCase {
    ...
    func testAccessibilityLabels() {
        ...
        // Naming convention: dot.notation, snake_case, or PascalCase are all acceptable
        for identifier in identifiers {
            XCTAssertFalse(identifier.isEmpty, "Accessibility identifier should not be empty: \(identifier)")
            let isValidFormat = identifier.contains(".") || identifier.contains("_") || identifier.first?.isUppercase == true
            XCTAssertTrue(isValidFormat, "Identifier should follow naming convention: \(identifier)")
        }
    }
    ...
    func testSliderDebouncing() async {
        // Simulate debouncing with proper cancellation
        var volume: Float = 0.5
        var pendingTask: Task<Void, Never>?
        var saveCount = 0

        // Debounce logic: cancel previous task, schedule new one
        ...
        // Verify only the final value was saved
        XCTAssertEqual(volume, 0.8, "Volume should be set to last value")
        XCTAssertEqual(saveCount, 1, "Only one save should occur due to debouncing")
    }
