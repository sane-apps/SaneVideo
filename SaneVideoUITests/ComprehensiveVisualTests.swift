...
final class ComprehensiveVisualTests: XCTestCase {
    ...
    func handleSystemAlerts(timeout: TimeInterval = 5) -> Bool {
        ...
        if app.alerts.count > 0 {
            ...
            if alert.exists {
                ...
                if alertText.contains("microphone") || alertText.contains("camera") || alertText.contains("screen") {
                    if alert.buttons["Allow"].exists {
                        alert.buttons["Allow"].tap()
                    } else if alert.buttons["OK"].exists {
                        alert.buttons["OK"].tap()
    ...
    func testAppCrashRecovery() throws {
        ...
        if window.waitForExistence(timeout: 5) {
            ...
            XCTAssertTrue(app.state == .runningForeground, "App should remain responsive after crash recovery")
        } else {
    ...
    func testTimelineUI() throws {
        ...
        if timeline.waitForExistence(timeout: 5) {
            ...
            XCTAssertTrue(emptyState.exists || timeline.exists, "Timeline or empty state should be visible")
        }
    }
    ...
    func testUIResponsiveness() throws {
        ...
        XCTAssertTrue(duration < 20, "App should load within 20 seconds (including permission handling)")
    }
    ...
    func testSystemPermissionDialogs() throws {
        ...
        if systemAlerts.count > 0 {
            ...
            if alert.exists {
                ...
                if alertText.contains("camera") || alertText.contains("microphone") || alertText.contains("screen") {
                    if alert.buttons["Allow"].exists {
                        alert.buttons["Allow"].tap()
                    } else if alert.buttons["OK"].exists {
                        alert.buttons["OK"].tap()
