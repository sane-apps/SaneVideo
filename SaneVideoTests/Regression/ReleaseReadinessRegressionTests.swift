...
final class ReleaseReadinessRegressionTests: XCTestCase {
    ...
    func testPurchaseAndPermissionCopyAvoidsV1PromiseDrift() throws {
        ...
        XCTAssertTrue(appSource.contains("No trial and no paywall"))
        XCTAssertTrue(appSource.contains("Donate only if you want to support it"))
        XCTAssertTrue(appSource.contains("donationURL: OpenSourceRelease.donationURL"))
        XCTAssertFalse(appSource.contains("14-day Pro trial"))
        XCTAssertFalse(appSource.contains("Pro is required after the 14-day trial"))
        XCTAssertFalse(appSource.contains("Basic stays available after the trial"))
        XCTAssertFalse(appSource.contains("Keep Pro for $14.99 once"))
    ...
    func testAppStoreScreenshotStoryboardAndGeneratorCoverLaunchSellingPoints() throws {
        ...
        XCTAssertTrue(website.contains("Every step runs on your machine."))
        XCTAssertTrue(website.contains("Edit a meeting walkthrough with a real timeline."))
        XCTAssertTrue(website.contains("Magic Fix cleanup, on device"))
        XCTAssertTrue(website.contains("Native timeline + Magic Fix"))
        ...
        XCTAssertTrue(website.contains("Your files stay on your Mac"))
