...
extension RecordingEngine {
    ...
    func setupSoundAnalysisMonitoring() {
        Task {
            for await classification in soundAnalysisService.resultsStream.values where classification.confidence > 0.7 {
                AppLogger.recording.info("🎯 Detected sound: \(classification.label.displayName) (\(Int(classification.confidence * 100))%)")
            }
        }
    }
    ...
    func setupSubscriptions() {
        ...
        // 4. Microphone (Combined SCStream or standalone)
        Task { @RecordingActor [weak self] in
            guard let self = self else { return }
            for await buffer in screenRecorder.micSampleBufferSubject.values where currentSource == .screen {
        ...
        Task { @RecordingActor [weak self] in
            guard let self = self else { return }
            for await buffer in audioService.sampleBufferSubject.values where currentSource != .screen {
                self.processAudioSample(buffer)
            }
        }
