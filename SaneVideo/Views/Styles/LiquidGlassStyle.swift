...
extension View {
    ...
    /// Capsule-shaped liquid glass for pill-shaped controls (recording bars, etc.)
    func capsuleGlass(intensity: LiquidGlassModifier.GlassIntensity = .standard) -> some View {
        self.modifier(CapsuleLiquidGlassModifier(intensity: intensity))
    }
}

/// A view modifier that applies capsule-shaped Liquid Glass aesthetic.
/// Used for pill-shaped controls like recording bars and floating controls.
struct CapsuleLiquidGlassModifier: ViewModifier {
    var intensity: LiquidGlassModifier.GlassIntensity = .standard
...
/// Conditional capsule glass - only applies the effect when enabled.
/// Use this when the glass background is optional based on configuration.
struct ConditionalCapsuleGlass: ViewModifier {
    let isEnabled: Bool

    func body(content: Content) -> some View {
        if isEnabled {
            content.modifier(CapsuleLiquidGlassModifier(intensity: .standard))
        } else {
            content
        }
    }
