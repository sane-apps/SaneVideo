...
final class SaneVideoCompositor: NSObject, AVVideoCompositing {
    ...
    private func render(_ request: AVAsynchronousVideoCompositionRequest) async {
        guard let instruction = request.videoCompositionInstruction as? SaneVideoCompositionInstruction,
            ...
            if let outputPixelBuffer = request.renderContext.newPixelBuffer() {
                ...
                request.finish(with: NSError(domain: "SaneVideoCompositor", code: -1, userInfo: [NSLocalizedDescriptionKey: "Empty instructions"]))
            }
            return
        }

        guard let outputPixelBuffer = request.renderContext.newPixelBuffer() else {
            request.finish(with: NSError(domain: "SaneVideoCompositor", code: -2, userInfo: [NSLocalizedDescriptionKey: "Failed to create output buffer"]))
            return
        }
        ...
        for layerInstruction in instruction.layerInstructions.reversed() {
            ...
            // Transitions
            if let transitionMeta = instruction.activeTransitions.first(where: { $0.fromTrackID == trackID || $0.toTrackID == trackID }) {
                processedTrackIDs.insert(transitionMeta.fromTrackID)
                processedTrackIDs.insert(transitionMeta.toTrackID)

                if let fromImage = await getLayerImage(for: transitionMeta.fromTrackID, request: request, instruction: instruction),
                   let toImage = await getLayerImage(for: transitionMeta.toTrackID, request: request, instruction: instruction) {
                    ...
                    layerOutput = transitionModel.apply(from: fromImage, to: toImage, progress: CGFloat(progress))
                } else {
                    if let toImg = await getLayerImage(for: transitionMeta.toTrackID, request: request, instruction: instruction) {
                        layerOutput = toImg
                    } else {
                        layerOutput = await getLayerImage(for: transitionMeta.fromTrackID, request: request, instruction: instruction)
                    }
                }
        ...
        if let resultImage = currentImage {
            let activeTextLayers = instruction.textLayers.filter { $0.timeRange.containsTime(request.compositionTime) }
            if !activeTextLayers.isEmpty {
                ...
                let shouldDetectFaces = activeTextLayers.contains(where: { $0.isCaption }) && 
                                      !ThermalManager.isThrottled
                ...
                if let textImage = TextLayerRenderer.renderTextLayers(activeTextLayers, size: renderSize, faceRects: faceRects) {
        ...
        // 4. Render Cursor
        for (_, cursorURL) in instruction.trackCursorData {
             let validTime = request.compositionTime.seconds
             // Ideally we need start time offset here, but simplified for MVP
             if let cursorImage = CursorRenderer.renderCursor(for: cursorURL, time: validTime, renderSize: request.renderContext.size) {
    ...
    private func getLayerImage(for trackID: CMPersistentTrackID, request: AVAsynchronousVideoCompositionRequest, instruction: SaneVideoCompositionInstruction) async -> CIImage? {
        ...
        guard let layerInstruction = instruction.layerInstructions.first(where: { $0.trackID == trackID }) else { return layerImage }

        // A. Apply Transform (Base Layer Instruction)
        var startTransform = CGAffineTransform.identity
        if layerInstruction.getTransformRamp(for: request.compositionTime, start: &startTransform, end: nil, timeRange: nil) {
        ...
        if let keyframeRanges = instruction.trackKeyframes[trackID] {
            for (range, animations) in keyframeRanges where range.containsTime(request.compositionTime) {
                ...
                for animation in animations {
                    applyKeyframes(animation, relativeTime: relativeTime, renderSize: request.renderContext.size, to: &layerImage)
                }
            }
        }
            ...
            if !ThermalManager.isEmergency {
                for (range, bgEffects) in bgEffectRanges where range.containsTime(request.compositionTime) {
                    for bgEffect in bgEffects {
                        await applyBackgroundEffect(bgEffect, to: &layerImage, visionService: instruction.visionService)
                    }
                }
            } else {
    ...
    private func applyKeyframes(_ animation: KeyframeAnimation, relativeTime: CMTime, renderSize: CGSize, to image: inout CIImage) {
    ...
    private func applyVideoEffect(_ effect: VideoEffect, to image: inout CIImage) {
        if effect.type == .autoEnhance {
             let options: [CIImageAutoAdjustmentOption: Any] = [.enhance: true, .redEye: false, .features: []]
    ...
    private func applyBackgroundEffect(_ bgEffect: BackgroundEffect, to image: inout CIImage, visionService: PersonSegmentationService?) async {
        guard let visionService = visionService else { return }
        
        switch bgEffect {
        case .blur(let radius):
            if let blurred = try? await visionService.applyBackgroundBlur(to: image, blurRadius: radius, reuseRequest: true) { image = blurred }
        case .solidColor(let r, let g, let b, let a):
            let color = NSColor(red: r, green: g, blue: b, alpha: a)
            if let replaced = try? await visionService.replaceBackground(in: image, with: color, reuseRequest: true) { image = replaced }
        case .image(let url):
            if let bgCI = CIImage(contentsOf: url) {
                if let replaced = try? await visionService.replaceBackground(in: image, with: bgCI, reuseRequest: true) { image = replaced }
            }
        }
    }
}
